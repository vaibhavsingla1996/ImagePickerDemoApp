...
open class GSImageViewerController: UIViewController {
    ...
    override open func viewDidLoad() {
        ...
        let cancelButton = UIButton.init(frame: CGRect.init(x: 15, y: UIApplication.shared.statusBarFrame.size.height + 15, width: 100, height: 30))
        cancelButton.setTitle("Cancel", for: .normal)
        cancelButton.titleLabel?.textColor = .white
        cancelButton.addTarget(self, action: #selector(cancelButtonTapped(sender:)), for: UIControlEvents.touchUpInside)
